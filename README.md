# JunkBlade Animation Speed Controller

**JunkBlade Simulator 専用の Chrome 拡張機能**です。`https://tsd0313.github.io/ygo-JunkBlade/dist/` 上で使われる CreateJS のトゥイーンを、**1x〜16x** の倍率で高速化します。動画の再生速度を変える拡張機能では対応できない、Canvas 内のカード移動・フェード・待機演出を対象にしています。

> 推奨設定は **4x** です。演出の確認を残したい場合は 2x、周回・検証用途には 8x を試してください。16x は画面遷移が急になり、操作や表示を見落とす可能性があります。

|項目|内容|
|---|---|
|対象サイト|[JunkBlade Simulator](https://tsd0313.github.io/ygo-JunkBlade/dist/)|
...
導入後、JunkBlade Simulator を開き、ツールバーの拡張機能アイコンから **JunkBlade Animation Speed** を開いてください。倍率を選び、**「この倍率を適用」**を押します。以降に開始する操作と、新たに発生する演出がその倍率になります。動いている最中の CreateJS 演出がある場合にも、その時点で反映を試みます。
...
対象ページはカード UI を Canvas 上に描画し、CreateJS の `Tween` を使って演出を進めています。実行時に確認したページでは、`createjs.Tween` と `createjs.Ticker` が読み込まれており、TweenJS の `timeScale` プロパティを 4 にしたテストでは、100ms の進行が通常約99ms相当から約399ms相当へ進みました。TweenJS の公式 API でも `timeScale` はトゥイーンの設定項目として定義されています。[2]
...
|検証項目|結果|
|---|---|
|対象ページの実装|`#canv` 等の Canvas と `createjs.Tween` を確認|
|速度制御の根拠|TweenJS の `timeScale` を確認|
|倍率動作の簡易検証|100ms の手動進行で 1x は約99、4x は約399 に進行|
|新規トゥイーンへの適用|4x が自動設定されることを確認|
|拡張機能ファイル|`manifest.json` と JavaScript の構文検証済み|
